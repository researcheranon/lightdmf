...
def get_modality_dims(sample, feature_names, feature_dir=None, n_qwen3_features=None, metadata=None):
	"""Get modality dimensions.
	
	Args:
		sample: A sample row from metadata (used as fallback)
		feature_names: Dict mapping modality to feature name
		feature_dir: Directory containing features (only used for non-merged datasets)
		n_qwen3_features: Number of qwen3 features to use
		metadata: Full metadata DataFrame (optional, used to find samples with specific modalities)
	"""
	modality_dims = {}
	...
	for modality, feature_name in feature_names.items():
		...
		# Find an appropriate sample for this modality by checking if feature file exists
		sample_to_use = None
		
		if metadata is not None:
			# Try to find a sample that has this modality's feature file
			if modality == 'text' and 'unique_utterance' in metadata.columns:
				candidate_samples = metadata[metadata['unique_utterance'] == True]
			elif modality == 'audio':
				if 'unique_audio' in metadata.columns:
					candidate_samples = metadata[metadata['unique_audio'] == True]
				else:
					# If unique_audio column doesn't exist, assume all samples have audio except GoEmotions
					if 'dataset_name' in metadata.columns:
						candidate_samples = metadata[metadata['dataset_name'] != 'GoEmotions']
					else:
						candidate_samples = metadata
			else:
				candidate_samples = metadata
			
			# Search through candidates to find one with existing feature file
			for _, candidate in candidate_samples.iterrows():
				filestem = Path(candidate['file_path']).stem
				
				# Determine the correct feature directory for this candidate
				if feature_dir is None or 'dataset_dir' in candidate:
					if 'dataset_dir' in candidate:
						candidate_feature_dir = Path(candidate['dataset_dir']) / 'features'
					else:
						continue  # Skip if we can't determine feature dir
				else:
					candidate_feature_dir = Path(feature_dir)
				
				feature_path = candidate_feature_dir / feature_name / f"{filestem}.npy"
				
				if feature_path.exists():
					sample_to_use = candidate
					break
		
		# Fallback to the original sample if no suitable sample found
		if sample_to_use is None:
			sample_to_use = sample
		
		filestem = Path(sample_to_use['file_path']).stem
		
		# Determine the correct feature directory for this specific sample
		if feature_dir is None or 'dataset_dir' in sample_to_use:
			if 'dataset_dir' in sample_to_use:
				sample_feature_dir = Path(sample_to_use['dataset_dir']) / 'features'
			else:
				raise ValueError("feature_dir must be provided when sample doesn't contain dataset_dir")
		else:
			sample_feature_dir = Path(feature_dir)
		
		feature_path = sample_feature_dir / feature_name / f"{filestem}.npy"
		
		if not feature_path.exists():
			raise FileNotFoundError(f"Feature file not found: {feature_path}. Cannot determine dimension for {modality} modality. Make sure features are extracted for at least one sample with this modality.")
...
class FeatureDatasetLazy(Dataset):
	def __init__(self, 
		...
		if split != 'train':
			# Ensure each filename appears only once for non-train splits.
			self.metadata = self.metadata.drop_duplicates(subset=['file_path'], keep='first').reset_index(drop=True)
		...
		if idx_with_text.any():     
			...
			sample = self.metadata.iloc[0]
		self.modality_dims = get_modality_dims(sample, self.feature_names, feature_dir=self.feature_main_dir, n_qwen3_features=self.n_qwen3_features, metadata=self.metadata)
